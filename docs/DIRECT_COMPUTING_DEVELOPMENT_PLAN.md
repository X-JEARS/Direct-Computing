...
实现语言：Rust  
许可证方向：开源项目，最终许可证待确定；如复用 AGPL 代码，应按其许可证发布
...
阶段 0 已于 2026-09-20 完成：

- Rust Workspace 与规划中的 crate 结构
- 基础错误类型和日志入口
- 协议版本、能力和服务类型
- `direct-computing` 与 `dc-cli` 最小二进制
- Windows、Linux、macOS 持续集成任务
- Apache-2.0 许可证与第三方许可证记录

下一次开发建议进入阶段 1：

> 定义帧、像素格式、编码包和时间戳等媒体数据模型；定义捕获、编码、解码和显示接口；先使用合成测试画面建立可测试的本地回环管线，再接入首个平台的屏幕采集和 H.264 软件编解码器。
