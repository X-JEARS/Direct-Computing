...
- [ ] 在 Windows 10/11 真机验证采集、颜色、方向和多显示器行为
- [ ] 接入本地图形窗口显示解码帧
- [ ] 建立持续运行、帧率和资源占用基准
...
编解码回环和 Windows DXGI 采集实现。Windows 代码已通过交叉编译，尚待 Windows 真机运行验证；
本地图形窗口显示也尚未完成。

下一次开发从阶段 1 的运行验证和显示接入继续：

> 按 `docs/WINDOWS_CAPTURE_TESTING.md` 在 Windows 10/11 真机验证 DXGI 采集结果；根据结果修正
> 显示器枚举、旋转和访问丢失处理。随后建立跨平台最小显示窗口，将解码帧直接显示，并补充
> 连续运行和资源基准。
